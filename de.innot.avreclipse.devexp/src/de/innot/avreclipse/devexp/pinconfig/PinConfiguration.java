...
public class PinConfiguration extends Composite {
	...
	public PinConfiguration(Composite parent, int style) {
		...
		configData = new PinConfigData[]{ 
				new PinConfigData("PORTA", "PA0", false, false, "LED"), 
				new PinConfigData("PORTB", "PB6", true,false, "SD_CS"),
                new PinConfigData("PORTB", "PB7", false, true ,"SD_MOSI")};
		tableViewer.setInput(configData);		
	}
	
	private boolean loadConfigData(PinConfigData configData) {
		String configPath;
		configPath = projectPath + "/.settings/pins.xml";
		//iterate trought pins, and select... GPIO only
		return false; // error
	}
	
	private boolean saveConfigData(PinConfigData configData) {
		String configPath;
		configPath = projectPath + "/.settings/pins.xml";
